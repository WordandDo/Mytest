...
# 路径配置
SEEDS_FILE = "/home/a1/sdb/lb/Mytest/src/data_synthesis/sample_entities_100.json"
OUTPUT_BASE = f"synthesis_results_simple_answer_{time.strftime('%Y%m%d_%H%M%S')}_100seeds"
LOG_DIR = Path("logs")
LOG_DIR.mkdir(exist_ok=True)

# 任务定义：模式、合成配置、工具白名单（仅暴露检索相关工具 + 会话初始化）
TASKS = [
    {
        "mode": "hybrid",
        "synthesis_config": "src/data_synthesis/configs/rag_config_hybrid.json",
        "tool_whitelist": [
            "setup_rag_session",
            "query_knowledge_base_dense",
            "query_knowledge_base_sparse"
        ]
    },
    {
        "mode": "denseonly",
...
def main():
    ...
    try:
        ...
        # 3. 并发启动合成任务（共享同一 Gateway）
        for task in TASKS:
            mode = task["mode"]

            print(f"\n\n{'='*60}")
            print(f"🌊 Processing Workflow (parallel): {mode.upper()}")
            print(f"{'='*60}")
